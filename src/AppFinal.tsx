...
type Modal = 'baixar' | 'negocio' | 'motorista' | 'admin' | 'painel' | null
...
export default function AppFinal() {
  ...
  return <main className="app-shell">
    ...
    <section className="hero" id="top">
      ...
      <Phone ev={ev} cat={cat} choose={choose} tab={tab} setTab={setTab} q={q} setQ={setQ} will={will} abrirWill={abrirWill} ride={ride} setRide={setRide} react={react} setReact={setReact} panel={panel} setPanel={setPanel} open={setModal} />
    </section>
...
function Phone({ ev, cat, choose, tab, setTab, q, setQ, will, abrirWill, ride, setRide, react, setReact, panel, setPanel, open }: { ev: Experience; cat: Category; choose: (c: Category) => void; tab: Tab; setTab: (t: Tab) => void; q: string; setQ: (v: string) => void; will: () => void; abrirWill: () => void; ride: string; setRide: (v: string) => void; react: string; setReact: (v: string) => void; panel: boolean; setPanel: (v: boolean) => void; open: (m: Modal) => void }) {
...
function Section({ id, eyebrow, title, text, cards, cta, onCta }: { id: string; eyebrow: string; title: ReactNode; text: string; cards: string[][]; cta: string; onCta: () => void }) {
  return <section id={id} className="public-section"><span className="pill">{eyebrow}</span><h2>{title}</h2><p>{text}</p><div className="public-cards">{cards.map(([i, t, p]) => <article key={t}><span>{i}</span><h3>{t}</h3><p>{p}</p></article>)}</div><div className="cta-box"><h3>{cta}</h3><p>Toque para seguir a jornada e abrir o próximo passo dentro da plataforma.</p><button className="secondary" onClick={onCta}>{cta}</button></div></section>
}

function ModalBox({ modal, setModal }: { modal: Modal; setModal: (m: Modal) => void }) {
